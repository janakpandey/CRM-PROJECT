...
In Django, models are Python classes that define the structure of your database tables. For our CRUD project, let's assume we want to manage a list of orders. Create a model for the orders in `RecordApp/models.py`:
...
We mentioned using a form for creating and updating orders. You can define the form in `RecordApp/forms.py`:
...
In `RecordApp/views.py`, define a view function for creating a new order:
...
def orderFormView(request):
    ...
    template_name = 'RecordApp/order.html'
    context = {'form': form}
...
Now, let's create a view to display a list of all books in `RecordApp/views.py`:

```python
def showView(request):
    obj = Orders.objects.all()
    template_name = 'RecordApp/show.html'
    context = {'obj': obj}
    return render(request, template_name, context)
```
...
To update a order, create a view in `RecordApp/views.py`:
...
Finally, let's create a view to delete a order in `RecordApp/views.py`:
...
def deleteView(request, f_oid):
    ...
    if request.method == 'POST':
        obj.delete()
        return redirect('show_url')
    template_name = 'RecordApp/confirmation.html'
    context = {'obj': obj}
    return render(request, template_name, context)
```
...
Similarly, create HTML templates for the views in the crudproject/templates/RecordApp directory. You'll need templates for the following views:
...
`crudproject/templates/RecordApp/orders.html`
...
`crudproject/templates/RecordApp/show.html`
...
{% endblock %}
 ...
 `crudproject/templates/RecordApp/confirmation.html`
...
Finally, configure the URLs for your views. In your project's crudproject/urls.py file, include the URLs for the RecordApp app:
...
Then, in your app's RecordApp/urls.py file, define the URLs for your views:
